...
static const std::map<llm_arch, std::map<llm_tensor, std::string>> LLM_TENSOR_NAMES = {
    ...
    },
    {
        LLM_ARCH_KIMI_K3,
        {
            ...
            { LLM_TENSOR_ATTN_NORM,              "blk.%d.attn_norm" },
            { LLM_TENSOR_FFN_NORM,               "blk.%d.ffn_norm" },
            // AttnRes, one folded [n_embd] vector per site, two sites per layer
            { LLM_TENSOR_ATTN_RES_SCORE,         "blk.%d.attn_res_score" },
            { LLM_TENSOR_FFN_RES_SCORE,          "blk.%d.ffn_res_score" },
            // KDA layers: q/k/v are ordinary projections, the rest is the
            // delta-net machinery. Three separate short convs, unlike
            // Qwen3-Next's single fused one.
            { LLM_TENSOR_ATTN_Q,                 "blk.%d.attn_q" },
            { LLM_TENSOR_ATTN_K,                 "blk.%d.attn_k" },
            { LLM_TENSOR_ATTN_V,                 "blk.%d.attn_v" },
            { LLM_TENSOR_ATTN_OUT,               "blk.%d.attn_output" },
            ...
            // full-attention layers: DeepSeek-style MLA plus the output gate
            { LLM_TENSOR_ATTN_Q_A,               "blk.%d.attn_q_a" },
            { LLM_TENSOR_ATTN_Q_A_NORM,          "blk.%d.attn_q_a_norm" },
            { LLM_TENSOR_ATTN_Q_B,               "blk.%d.attn_q_b" },
            { LLM_TENSOR_ATTN_KV_A_MQA,          "blk.%d.attn_kv_a_mqa" },
            ...
            { LLM_TENSOR_ATTN_V_B,               "blk.%d.attn_v_b" },
            { LLM_TENSOR_ATTN_GATE,              "blk.%d.attn_gate" },
            // dense FFN (blk.0 only)
            { LLM_TENSOR_FFN_GATE,               "blk.%d.ffn_gate" },
            { LLM_TENSOR_FFN_UP,                 "blk.%d.ffn_up" },
            { LLM_TENSOR_FFN_DOWN,               "blk.%d.ffn_down" },
            // latent MoE: router and shared experts at the full residual width,
            // routed experts at the narrower latent width, with down/up either side
            { LLM_TENSOR_FFN_GATE_INP,           "blk.%d.ffn_gate_inp" },
            { LLM_TENSOR_FFN_EXP_PROBS_B,        "blk.%d.exp_probs_b" },
