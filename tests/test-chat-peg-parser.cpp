...
// Kimi-K3's chat format, exercised OFFLINE against a string captured from the
// server with --reasoning-format none.
//
// Emission is a nestable section format,
//   <|open|>NAME<|sep|> BODY <|close|>NAME<|sep|>
// and the template's generation prompt ends with "<|open|>think", so the model
// RESUMES mid-section: its first emitted token is <|sep|>, not <|open|>.
//
// Deliberately NO p.prefix. prefix(s, delim) yields
// literal(s.substr(0, s.rfind(delim))) - the generation-prompt text BEFORE the
// delimiter, for models that re-echo their prompt. K3 does not echo, and its
// prompt has content before "<|open|>think", so a prefix rule demanded output
// the model never produces and the whole parse fell through. That was the bug in
...
static void test_kimi_k3_parser(testing & t) {
    ...
    auto parser = build_chat_peg_parser([&](common_chat_peg_builder & p) -> common_peg_parser {
        auto reasoning = p.optional("<|sep|>" + p.reasoning(p.until(THINK_CLOSE)) + THINK_CLOSE);
        return reasoning << RESP_OPEN << p.content(p.until(RESP_CLOSE)) << p.rest();
    });

    const std::string input =
        "<|sep|>The user asks for the capital of Japan in one word. The answer is Tokyo."
        "<|close|>think<|sep|><|open|>response<|sep|>Tokyo.<|close|>response<|sep|><|close|>message<|sep|>";

    common_peg_parse_context ctx(input, COMMON_PEG_PARSE_FLAG_NONE);
    auto result = parser.parse(ctx);
    if (!t.assert_equal("k3 parse ok", false, result.fail())) {
    ...
    t.log("Reasoning: " + msg.reasoning_content);
    t.log("Content  : " + msg.content);
    t.assert_equal("k3 content", std::string("Tokyo."), msg.content);
